...
from corehq.apps.commtrack.models import SupplyPointCase, CommTrackUser
...
def third_soh_to_super():
    ...
    for facility in facilities:
        ...
        missing_products = StockTransaction.objects.filter(case_id=facility._id, type='stockeout')

        if not on_time_products:
            for user in CommTrackUser.by_domain(DOMAIN):
                if user.is_active and user.location and user.location._id == facility._id:
...
def reminder_to_visit_website():
    for user in CommTrackUser.by_domain(DOMAIN):
        if user.is_active and user.last_login < datetime.datetime.now() - datetime.timedelta(weeks=13) \
                and user.get_verified_number():
