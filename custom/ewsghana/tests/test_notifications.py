...
class MissingReportNotificationTestCase(TestCase):
    ...
    def test_product_type_filter(self):
        ...
        bootstrap_web_user(
            username='test2', domain=self.TEST_DOMAIN, phone_number='+44445', location=self.district,
            email='test2@example.com', password='dummy', program_id=self.program.get_id
        )
...
class UrgentStockoutNotificationTestCase(TestCase):
    ...
    def test_product_type_filter(self):
        ...
        bootstrap_web_user(
            username='test2', domain=self.TEST_DOMAIN, phone_number='+44445', location=self.region,
            email='test2@example.com', password='dummy', program_id=program.get_id
        )
...
class SMSNotificationTestCase(TestCase):
    ...
    def setUp(self):
        self.district = make_loc('test-district', 'Test District', self.TEST_DOMAIN, 'district')
        self.user = bootstrap_web_user(
            username='test', domain=self.TEST_DOMAIN,
            phone_number='+4444', sms_notifications=True,
            email='test@example.com', location=self.district, password='dummy'
        )
    ...
    def test_opt_out(self):
        ...
        set_sms_notifications(self.user, False)
        with mock.patch('custom.ewsghana.alerts.alert.send_sms_to_verified_number') as send:
            self.notification.send()
            self.assertFalse(send.called)
