    ...
    if runner.location:
        if runner.location.location_type != 'FACILITY':
            ...
            non_facilities = itertools.dropwhile(
                ...
                filter(lambda location: location.location_type in non_facilities_types, non_facilities)
            )
        else:
            facilities = itertools.dropwhile(
                lambda location: location._id != runner.location.location_id,
                filter(lambda location: location.location_type == 'FACILITY', facilities)
            )
            non_facilities = filter(
                lambda location: location.location_type in non_facilities_types,
                non_facilities
            )
